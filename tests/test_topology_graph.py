...
class TopologyGraphTests(unittest.TestCase):
    ...
    def test_topology_service_preserves_impact_and_exposes_graph_api(self) -> None:
        ...
        upstream = client.get("/topology/rabbitmq/upstream")
        self.assertEqual(upstream.status_code, 200)
        self.assertEqual(upstream.json()["service_name"], "rabbitmq")
        self.assertEqual(upstream.json()["upstream"], ["queue-master", "shipping"])
        downstream = client.get("/topology/orders/downstream")
        self.assertEqual(downstream.status_code, 200)
        self.assertEqual(downstream.json()["service_name"], "orders")
        self.assertIn("shipping", downstream.json()["downstream"])
