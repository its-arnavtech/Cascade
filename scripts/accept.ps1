...
function Write-Phase9Debug {
    Write-Section "Command Center UI Debug Output"
...
try {
    Write-Section "Command Center UI Acceptance"
    ...
    $investigation = HttpJson POST "$base/api/agent/investigations" @{
        ...
        objective = "Command Center UI acceptance deterministic investigation"
        mode = "deterministic"
        max_steps = 8
    }
    ...
    $remPlan = HttpJson POST "$base/api/remediation/recommender/plans" @{
        ...
        service = $TargetService
        namespace = $TargetNamespace
        objective = "Command Center UI acceptance remediation plan"
    ...
    $approval = HttpJson POST "$base/api/remediation/approval/approvals" @{
        ...
        decision = "approved"
        approver = "command-center-acceptance"
    ...
    $chaosPlan = HttpJson POST "$base/api/chaos/planner/plans" @{
        objective = "Command Center UI acceptance chaos dry-run plan"
...
Write-Host ""
if ($Failed.Count -eq 0) { Write-Host "COMMAND CENTER ACCEPTANCE: PASS"; exit 0 }
Write-Host "COMMAND CENTER ACCEPTANCE: FAIL"
exit 1
