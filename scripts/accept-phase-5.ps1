...
try {
    Write-Section "Phase 5 Acceptance"
    ...
    try {
        ...
        if ($null -ne $counts.knowledge_documents -and $null -ne $counts.qdrant_knowledge_points) { Add-Pass "retrieval-service /debug/counts includes Phase 5 counts" } else { Add-Fail "Phase 5 counts missing from retrieval debug" }
    } finally { Stop-PF }
} catch {
...
Write-Host ""
if ($Failed.Count -eq 0) { Write-Host "PHASE 5 ACCEPTANCE: PASS"; exit 0 }
Write-Host "PHASE 5 ACCEPTANCE: FAIL"
exit 1
