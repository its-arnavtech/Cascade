...
function Cleanup-Chaos { if($script:ChaosApplied){ Invoke-Kubectl @("delete","-f","scripts\pod-kill-catalogue.yaml","--ignore-not-found=true") | Out-Null; $script:ChaosApplied=$false } }
...
try {
    ...
    $targetPods=Get-KubectlJson @("-n",$TargetNamespace,"get","pods","-o","json"); if($null -ne $targetPods -and @($targetPods.items|Where-Object{$_.status.phase -eq "Running"}).Count -gt 0){Add-Pass "Sock Shop pods Running"}else{Add-Fail "Sock Shop pods not Running"}
    ...
    $topicEnsure = Ensure-CascadeRedpandaTopics -Namespace $Namespace -Topics @("telemetry.raw","telemetry.enriched","experiments.events")
    if($topicEnsure.Success){Add-Pass "Telemetry Redpanda topics ensured"}else{Add-Fail "Telemetry Redpanda topics could not be ensured. Missing: $($topicEnsure.Missing -join ', ')"}
    ...
    Start-PortForward "observation-service" "8000:8000"; try { $h=Invoke-HttpJson GET "http://localhost:8000/health"; if($h.status -eq "ok"){Add-Pass "observation-service /health works"}else{Add-Fail "observation-service /health failed"}; $snap=Invoke-HttpJson GET "http://localhost:8000/snapshot" $null 20; $snapshotHasPods=($snap.namespace -eq "cascade-targets" -and @($snap.pods).Count -gt 0); if($snapshotHasPods){Add-Pass "observation-service /snapshot works"}else{Add-Fail "observation-service /snapshot failed or returned no pods"}; $raw=Invoke-HttpJson GET "http://localhost:8000/metrics/raw?query=up" $null 15; if($null -ne $raw.prometheus){Add-Pass "Prometheus reachable through observation-service"}elseif($snapshotHasPods){Add-Warn "Prometheus raw query failed, using Kubernetes snapshot fallback"}else{Add-Fail "Prometheus raw query failed"} } finally { Stop-PortForwards }
    ...
    Start-PortForward "experiment-tracker-service" "8002:8002"; try { $script:Experiment=Invoke-HttpJson POST "http://localhost:8002/experiments" @{experiment_type="pod-kill";target_service="catalogue";namespace="cascade-targets";duration_seconds=30;chaos_mesh_resource="kill-catalogue-once"} 10; if($null -ne $script:Experiment.experiment_id){Add-Pass "POST /experiments works"}else{Add-Fail "POST /experiments failed"} } finally { Stop-PortForwards }
    ...
    Start-PortForward "topology-service" "8004:8004"; try { $script:Impact=Invoke-HttpJson POST "http://localhost:8004/topology/impact" @{root_service="catalogue"} 10; if($null -ne $script:Impact.affected_services){Add-Pass "Topology impact works"}else{Add-Fail "Topology impact failed"} } finally { Stop-PortForwards }
