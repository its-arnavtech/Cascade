...
try {
    Write-Section "Phase 2 Acceptance"
...
Write-Host ""; if($Failed.Count -eq 0){Write-Host "PHASE 2 ACCEPTANCE: PASS"; exit 0}
Write-Host "Recommended next fix:"; if(($Failed -join " ") -match "Redpanda|Topic|telemetry"){Write-Host "  Run .\scripts\debug-phase-2-2.ps1 and inspect Redpanda/topics first."}else{Write-Host "  Fix the first failed service check above, then rerun acceptance."}
Write-Host "PHASE 2 ACCEPTANCE: FAIL"; exit 1
