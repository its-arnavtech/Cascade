...
try {
    Write-Section "Phase 4 Acceptance"
    ...
    try {
        ...
        if ($synthetic.window.window_id) { Add-Pass "Synthetic Phase 4 feature window inserted and labeled" } else { Add-Fail "Synthetic feature insert failed" }
    ...
    try {
        ...
        $detect = HttpJson POST "http://localhost:8014/detect" @{ lookback_minutes = 240; service = "phase4-synthetic-service"; publish = $true }
    ...
    try {
        ...
        if ($null -ne $counts.telemetry_feature_windows -and $null -ne $counts.anomaly_events -and $null -ne $counts.model_runs) { Add-Pass "retrieval-service /debug/counts includes Phase 4 counts" } else { Add-Fail "Phase 4 counts missing from retrieval debug" }
    } finally { Stop-PF }
} catch {
...
Write-Host ""
if ($Failed.Count -eq 0) { Write-Host "PHASE 4 ACCEPTANCE: PASS"; exit 0 }
Write-Host "PHASE 4 ACCEPTANCE: FAIL"
exit 1
