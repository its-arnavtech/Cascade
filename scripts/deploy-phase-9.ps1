...
function Write-Phase9Debug {
    Write-Section "Phase 9 Failure Debug"
...
try {
    Write-Section "Deploy Cascade Phase 9"
    foreach ($path in @(
        ...
        if (-not (Test-Path $path)) { throw "Required Phase 9 file missing: $path" }
    }
    ...
    if (-not $SkipPhase8Deploy) {
        Write-Section "Ensure Phase 8 Baseline"
        & .\scripts\deploy-phase-8.ps1 -ClusterName $ClusterName -Namespace $Namespace -SkipPhase7Deploy
        Assert-NativeSuccess "Deploy Phase 8 baseline"
    } else {
        Write-Host "Skipping Phase 8 deploy by flag; existing Phase 8 services must already be available."
    }
    ...
    Write-Section "Build Phase 9 Images"
    foreach ($image in @(
    ...
    Write-Section "Load Phase 9 Images Into kind"
    ...
    Write-Section "Apply Phase 9 Manifests"
    ...
    Write-Section "Wait For Phase 9 Rollouts"
    foreach ($deployment in @("command-center-api", "command-center")) {
    ...
    Write-Section "Smoke Test Phase 9 Services"
    ...
    Write-Section "Phase 9 Deploy Complete"
    ...
    Write-Host ".\scripts\accept-phase-9.ps1"
} catch {
    Write-Host "ERROR: $($_.Exception.Message)"
    try { Write-Phase9Debug } catch { Write-Host "WARN: Failed to collect Phase 9 debug output: $($_.Exception.Message)" }
    exit 1
} finally {
    Stop-PortForwards
}
