...
Phase 8 adds the remediation control plane for Cascade. It recommends safe next steps from real Cascade evidence, records human approval decisions, validates actions with dry-run checks, and blocks real execution by default.
...
Default Phase 8 workflows are dry-run only. `POST /executions/dry-run` stores a `remediation_executions` row with `dry_run=1` and `executed=0`.
...
This builds Phase 8 images, applies schema/topic jobs, deploys services/RBAC, and waits for rollouts.
...
Acceptance checks Phase 7 readiness, Phase 8 schema/topic, service readiness, plan creation, safety rejection, approvals, dry-run validation, real execution blocking, agent gateway tools, and list APIs.
...
- production hardening, rate limits, backups, and credential hardening are Phase 10.
