    ...
    <div className="page">
      ...
      <section className="panel wizard">
        <div className="wizard-steps">
          <button type="button" className={`wizard-step-pill ${step === 1 ? "active" : ""} ${completedPlanId ? "done" : ""}`} onClick={() => setStep(1)}>1 Create plan</button>
          <button type="button" className={`wizard-step-pill ${step === 2 ? "active" : ""} ${completedApprovalId ? "done" : ""}`} onClick={() => setStep(2)} disabled={!completedPlanId}>2 Record approval</button>
          <button type="button" className={`wizard-step-pill ${step === 3 ? "active" : ""}`} onClick={() => setStep(3)} disabled={!completedPlanId}>3 Dry-run validation</button>
        </div>
      ...
      <StatusPanel title="Remediation Plans" loading={plans.isLoading} error={plans.error}>
        <DataTable rows={plans.data?.plans ?? []} columns={[{ key: "created_at", label: "Created" }, { key: "plan_id", label: "Plan" }, { key: "service", label: "Service" }, { key: "action_type", label: "Action" }, { key: "status", label: "Status", render: (row) => <Badge tone={statusTone(row.status)}>{String(row.status ?? "-")}</Badge> }, { key: "action_summary", label: "Summary" }]} />
      </StatusPanel>
